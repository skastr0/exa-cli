...
const platformPackages = [
  { target: "darwin-arm64", packageDir: "packages/npm/exa-cli-darwin-arm64" },
  { target: "darwin-x64", packageDir: "packages/npm/exa-cli-darwin-x64" },
  { target: "linux-arm64", packageDir: "packages/npm/exa-cli-linux-arm64" },
  { target: "linux-x64", packageDir: "packages/npm/exa-cli-linux-x64" },
] as const
...
for (const { target, packageDir } of platformPackages) {
  const source = join(REPO_ROOT, "dist", `exa-cli-${target}`)
  const binDir = join(REPO_ROOT, packageDir, "bin")
  const destination = join(binDir, "exa-cli")
